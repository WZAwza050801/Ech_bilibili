...
> 恢复时对我说"继续跑批量"即可，我会执行 `python Ech_bilibili/batch_run.py`（后台）接着干。

## 0. 最新断点

- **目录结构整理（2026-09-22）**：各平台归各家 —— B 站全部内容（管线代码 + runs + 江左道卡卡-读书笔记）已从 `work/pipeline1/` 迁到 **`Ech_bilibili/`**，与 `Ech_youtube/` 平级；仓库根只留通用文档。所有脚本硬编码路径、`.gitignore`、`ech_config.py`（`ECH_BILI_DIR`）已同步更新
...
- **教训**：不要并发跑两个 batch（Ech_bilibili 根中转文件互相踩，踩过一次）
...
| 读书笔记批量 | ✅ 收官：111/114 完成，3 期充电专属（BV1bH4aeAE7E / BV1o4421Q7KG / BV1FJ4m1W71F），详见 Ech_bilibili/江左道卡卡-读书笔记/归档报告.md |

## 二、踩坑记录（重要）

1. **假产物链**：风控掐断音频下载 → 截断 m4s 被 ffmpeg"假成功"转换 → 假 wav 转写半截笔记。防线：三道校验（转写时长≥95% / polish 字数漂移±30% / 缓存命中复检）+ 截断产物移 `runs/_trash/`
2. **残留转写误 copy**：转写 subprocess 崩溃时上一视频的 transcript 被误当当前产物（261s 之谜=某 4:21 视频）。已修：转写前清 Ech_bilibili 根残留 + 严格校验
3. **OOM 两连**：昨晚 `mkl_malloc`（系统 commit 内存耗尽，重启解决）；今天 `bad allocation`（ctranslate2 线程 workspace 峰值，`cpu_threads=4` 解决）
4. **batch 并发互踩**：两个 batch 同时跑会抢 Ech_bilibili 根的 audio.wav/transcript.json → 永远串行
...
- 总控：`Ech_bilibili/pipeline1.py`（三道校验+rm回收）
- 批量：`Ech_bilibili/batch_run.py`（断点续跑+OOM重试）
- 转写：`Ech_bilibili/transcribe_local.py`（int8 + cpu_threads=4）
- 列表：`Ech_bilibili/video_list.json`（114个）
- 日志：`Ech_bilibili/江左道卡卡-读书笔记/batch_log.txt`
